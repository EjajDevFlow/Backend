...
if (!process.env.GOOGLE_GENERATIVE_AI_KEY) {
  console.error(
    "Google Generative AI API key is missing. Please set GOOGLE_GENERATIVE_AI_KEY in the .env file."
  );
  ...
  try {
    ...
    if (!title || !description || !classroomId || !primaryPdfUrl || !dueDate) {
      return res.status(400).json({
        message:
          "Missing required fields: title, description, classroomId, primaryPdfUrl, and dueDate are required",
      });
    }
...
export const evaluateAllSubmissions = async (req, res) => {
  try {
    ...
    if (assignment.createdBy !== adminId) {
      ...
      return res
        .status(403)
        .json({ message: "Not authorized to evaluate submissions" });
    }
...
- Ensure feedback tone is constructive and professional`;
        ...
        console.log(
          `Successfully evaluated submission ${submission._id} with score ${score}`
        );
    ...
    if (error.message.includes("API key not valid")) {
      console.error(
        "Invalid Google Generative AI API key. Please verify the key in the .env file."
      );
    }
